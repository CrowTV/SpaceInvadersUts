...
while running:
    for event in pygame.event.get():
        ...
        if event.type == shoot_laser and game.run and app_state == 'playing':
            game.alien_shoot_laser()

        if event.type == extra_point_alien and game.run and app_state == 'playing':
        ...
        if keys[pygame.K_ESCAPE]:
            if app_state == 'highscores':
                ...
                running = False
        # Solo permitir reiniciar con SPACE si no estamos pidiendo nombre y estamos en playing
        if keys[pygame.K_SPACE] and app_state == 'playing' and game.run == False and not game.await_highscore:
            game.reset()
